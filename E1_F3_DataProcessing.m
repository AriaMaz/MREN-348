...
for k = 1 : length(pos) % For every observation 'k'
   ...
   phi_f(:,1) = [vel(k,1); 0; 0; 0];        
   phi_f(:,2) = [sign(vel(k,1)); 0; 0; 0]; 
   phi_f(:,3) = [0; vel(k,2); 0; 0];              
   phi_f(:,4) = [0; sign(vel(k,2)); 0; 0]; 
   phi_f(:,5) = [0; 0; vel(k,3); 0];      
   phi_f(:,6) = [0; 0; sign(vel(k,3)); 0]; 
   phi_f(:,7) = [0; 0; 0; vel(k,4)]; 
   phi_f(:,8) = [0; 0; 0; sign(vel(k,4))]; 
